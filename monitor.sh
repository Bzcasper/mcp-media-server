...
class HealthMonitor:
    ...
    def check_connection_health(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Connection health check failed with status code: {response.status_code}")
                return False, {"status": "error", "http_status": response.status_code}
    ...
    def _handle_failure(self, reason):
        ...
        logger.warning(f"Health check failed ({self.failure_count}/{self.config['monitoring']['failure_threshold']}): {reason}")

        # Check if we've hit the failure threshold
        if self.failure_count >= self.config["monitoring"]["failure_threshold"]:
            logger.error(f"Failure threshold reached: {self.failure_count} consecutive failures")
    ...
    def _reboot_system(self):
        ...
        try:
            if sys.platform == "win32":
                subprocess.run(["shutdown", "/r", "/t", "30", "/c", "MCP Media Server health check initiated reboot"])
            else:
                subprocess.run(["sudo", "shutdown", "-r", "+1", "MCP Media Server health check initiated reboot"])
